...
st.markdown("""
    ...
    /* Phóng to font chữ của các thẻ số liệu cho rõ ràng */
    .big-info-value { color: #00FFFF; font-size: 24px; font-weight: bold; }
    </style>
...
def load_data():
    try:
        ...
        c_kyluc = find_col(['Kỷ Lục Sức Mạnh', 'Kỷ Lụcc'])
        ...
        # --- THAY ĐỔI TẠI ĐÂY: XẾP RANK CHỈ THEO KILL ---
        # Sắp xếp theo phần trăm hoàn thành Kill (K_PCT)
...
if res:
    ...
    tab1, tab2, tab3 = st.tabs(["👤 HỒ SƠ CHI TIẾT", "📊 BẢNG TỔNG HỢP", "🏆 VINH DANH (>100% KILL)"])
    ...
    with tab1:
        ...
        if sel:
            ...
            # Hiển thị Rank và các chỉ số quan trọng to rõ
            r1, r2, r3 = st.columns(3)
            with r1: st.markdown(f'<div class="info-box"><div class="info-label">🏆 HẠNG (THEO KILL)</div><div class="big-info-value">#{int(d["RANK"])}</div></div>', unsafe_allow_html=True)
            with r2: st.markdown(f'<div class="info-box"><div class="info-label">🎯 % KILL</div><div class="big-info-value">{d["K_PCT"]}%</div></div>', unsafe_allow_html=True)
            with r3: st.markdown(f'<div class="info-box"><div class="info-label">💀 % DEAD</div><div class="big-info-value">{d["D_PCT"]}%</div></div>', unsafe_allow_html=True)
    ...
    with tab2:
        st.subheader("📋 BẢNG TỔNG HỢP (XẾP HẠNG THEO % KILL)")
        view_df = df[['RANK', c_name, c_kyluc, c_kill, 'K_PCT', 'SUM_DEAD_UNITS', 'D_PCT']]
        st.dataframe(view_df.style.format({c_kyluc: '{:,.0f}', c_kill: '{:,.0f}', 'K_PCT': '{:.1f}%', 'SUM_DEAD_UNITS': '{:,.0f}', 'D_PCT': '{:.1f}%'}), 
                     use_container_width=True, height=600)

    with tab3:
        st.subheader("🔥 CHIẾN BINH HOÀN THÀNH >100% KILL")
        winner_df = df[df['K_PCT'] >= 100][['RANK', c_name, 'K_PCT', 'D_PCT']]
        if not winner_df.empty:
            st.balloons()
            st.table(winner_df.style.format({'K_PCT': '{:.1f}%', 'D_PCT': '{:.1f}%'}))
        else:
