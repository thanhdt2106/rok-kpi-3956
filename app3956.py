...
def load_data():
    try:
        df = pd.read_csv(URL)
        # Chuẩn hóa tên cột: xóa khoảng trắng thừa ở đầu/cuối
        df.columns = [str(c).strip() for c in df.columns]
        
        # Chuyển đổi dữ liệu số (bỏ qua cột tên và ID)
        numeric_cols = ['Sức Mạnh', 'Kỷ Lực Sức Mạnh', 'T5 tử vong', 'T4 tử vong', 'T3 tử vong', 
                        'T2 tử vong', 'T1 tử vong', 'Tổng Điểm Tiêu Diệt', 'Tài Nguyên Thu Thập',
                        'Tổng Tiêu Diệt T5', 'Tổng Tiêu Diệt T4', 'Tổng Tiêu Diệt T3', 
                        'Tổng Tiêu Diệt T2', 'Hỗ Trợ Liên Minh']
        ...
        st.error(f"Lỗi nạp dữ liệu hoặc sai tên cột: {e}")
        return None
...
if df is not None:
    ...
    with tab1:
        ...
        if sel:
            ...
            with ck1:
                fig_k = go.Figure(go.Indicator(mode="gauge+number", value=d['Tổng Điểm Tiêu Diệt'], title={'text': "TIẾN ĐỘ TIÊU DIỆT"}, gauge={'axis':{'range':[0, df['Tổng Điểm Tiêu Diệt'].max()*1.1]},'bar':{'color':"#58a6ff"}}))
