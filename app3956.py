...
st.markdown("""
    ...
    .info-box { background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 12px; text-align: center; margin-bottom: 8px; }
    ...
    .gauge-footer { color: #58a6ff; font-size: clamp(14px, 3.5vw, 18px); font-weight: 800; text-align: center; margin-top: -30px; }

    .big-table-container [data-testid="stDataFrame"] th { background-color: #1f2937 !important; color: #00FFFF !important; font-size: 18px !important; font-weight: 900 !important; }
    .big-table-container [data-testid="stDataFrame"] td { font-size: 17px !important; font-weight: 600 !important; }

    @media (max-width: 768px) {
        [data-testid="column"] { width: 48% !important; flex: 1 1 45% !important; min-width: 45% !important; }
        div[data-testid="stHorizontalBlock"] { gap: 5px !important; display: flex; flex-wrap: wrap; }
    }
    </style>
...
def load_data():
    try:
        ...
        df['SUM_DEAD'] = df[['T5 tử vong', 'T4 tử vong', 'T3 tử vong', 'T2 tử vong', 'T1 tử vong']].sum(axis=1, skipna=True)
...
if res:
    ...
    # 1. Hiển thị Header không có nút chọn ngôn ngữ
    st.markdown(f'<div class="main-header">{TEXTS["VN"]["header"]}</div>', unsafe_allow_html=True)
    
    # 2. Tạo Tabs
    tab1, tab2 = st.tabs([TEXTS["VN"]["tab1"], TEXTS["VN"]["tab2"]])
    
    with tab1:
        # Đưa chọn ngôn ngữ vào đây
        ...
        with st.expander(L["detail_title"]):
            ...
            for idx, (label, col_key) in enumerate(box_map):
                ...
                det_cols[idx % 5].markdown(f'<div class="info-box"><div class="info-label">{label}</div><div class="info-value" style="font-size:15px;">{txt}</div></div>', unsafe_allow_html=True)

        g1, g2 = st.columns(2)
        with g1:
            fig_k = go.Figure(go.Indicator(mode="gauge+number", value=d['K_PCT'], number={'suffix': "%"}, gauge={'bar': {'color': "#00FFFF"}}))
        ...
        with g2:
            fig_d = go.Figure(go.Indicator(mode="gauge+number", value=d['D_PCT'], number={'suffix': "%"}, gauge={'bar': {'color': "#f29b05"}}))
    ...
    with tab2:
        # Chọn ngôn ngữ cũng xuất hiện ở Tab 2 nếu cần
        lang2 = st.radio("🌐 Language Settings:", ["VN", "EN"], horizontal=True, label_visibility="collapsed", key="lang_tab2")
        L2 = TEXTS[lang2]
        ...
        v_df.columns = [L2['col_rank'], L2['col_name'], L2['col_power'], L2['col_kill'], L2['col_kpi_kill'], L2['col_dead'], L2['col_kpi_dead']]
        
        styled_df = v_df.style.format({L2['col_power']: '{:,.0f}', L2['col_kill']: '{:,.0f}', L2['col_dead']: '{:,.0f}', L2['col_kpi_kill']: '{:.1f}%', L2['col_kpi_dead']: '{:.1f}%'})
        styled_df = styled_df.map(lambda x: 'color: #00FF00;' if isinstance(x, (float, int)) and x >= 100 else '', subset=[L2['col_kpi_kill'], L2['col_kpi_dead']])
